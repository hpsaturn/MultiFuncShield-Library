...
/*
  24h Alarm Clock sample - adaptado para hpsaturn/MultiFuncShield

  button 1  : hold to set time or alarm
  button 2  : press to view alarm time or cancel alarm if in progress
  button 3  : increment hour / minute when setting (alarm) time.
              Hold to toggle alarm setting.

  LED1  : on = alarm enabled

  Ajuste feito:
  - Removido #include <TimerOne.h>
  - Removido Timer1.initialize()
  - Trocado MFS.initialize(&Timer1) por MFS.initialize()
*/
...
{
  /*
    Na biblioteca hpsaturn, initialize() não recebe TimerOne.
    A própria biblioteca já cuida da interrupção interna.
  */
